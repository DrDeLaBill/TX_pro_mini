...
void LCD5110::_print_char(unsigned char c, int x, int row)
{
	if (!_sleep)
	{
		...
		{
			for (int colmncnt = 0; colmncnt < cfont.x_size; colmncnt++) 
			{
				int font_idx = ((c - cfont.offset) * (cfont.x_size * (cfont.y_size / 8))) + 4;			
				
				for(int cnt=0; cnt < cfont.y_size / 8; cnt++)
				{
					_LCD_Write(PCD8544_SETXADDR | (x + colmncnt), LCD_COMMAND);
					_LCD_Write(PCD8544_SETYADDR | (row + cnt), LCD_COMMAND);
					if (cfont.inverted == 0)
						_LCD_Write(fontbyte(font_idx + cnt + (colmncnt * cfont.y_size / 8)), LCD_DATA);
					else
						_LCD_Write(~(fontbyte(font_idx + cnt + (colmncnt * cfont.y_size))), LCD_DATA);
				}
			}
